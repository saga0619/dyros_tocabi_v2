        ...
        for (int slave = 1; slave <= ec_slavecount; slave++)
        {
            if (ec_slave[slave - 1].state != EC_STATE_OPERATIONAL)
            {
                printf("%sELMO %d : EtherCAT State Operation Error : Slave %d State=0x%2.2x StatusCode=0x%4.4x : %s%s\n",
                       cred.c_str(), args.ecat_device, slave - 1, ec_slave[slave - 1].state, ec_slave[slave - 1].ALstatuscode,
...
void *ethercatThread1(void *data)
{
    ...
    TocabiInitArgs * init_args = data; 
    // force_control_mode = true;
    de_debug_level++;
    status_log = true;

    struct timespec ts_start;
    clock_gettime(CLOCK_MONOTOIC, &ts_start); //ADDED
    ...
    while (!shm_msgs_->shutdown)
    {
        ...
        for (int i = 0; i < ec_slavecount; i++)
        {
            ...
            if (elmost[i].state != elmost[i].state_before)
            {
                ...
                else
                {
                    ...
                    {
                        //printf("slave : " << i << " commutation check complete with operation enable 2\n");
                        elmost[i].commutation_ok = true;
        ...
        if (de_zp_sequence)
        {
            ...
            else if (init_args->ecat_device == 2) // Lower
            {
...
void *ethercatThread2(void *data)
{
    ...
    while (!de_shutdown)
    {
        ...
        if (ch != -1)
        {
            //std::printf("key input : " << (char)(ch % 256) << '\n';
            if ((ch % 256 == 'q'))
            {
                // std::printf("ELMO : shutdown request\n");
                de_shutdown = true;
            }
            else if ((ch % 256 == 'l'))
            {
                // std::printf("ELMO : start searching zero point lower\n");
                de_zp_lower_switch = true;
            }
            else if ((ch % 256 == 'u'))
            {
                // std::printf("ELMO : start searching zero point upper\n");
            ...
            else if ((ch % 256 == 'd'))
            {
                ...
                // std::printf("ELMO : debug mode, level : " << (int)de_debug_level << '\n';
            }
            else if ((ch % 256 == 'p'))
            {
                // std::printf("------------------------------------------------------\n");
            ...
            else if ((ch % 256 == 'w'))
            {
                ...
                // if (status_log)
                // {
                //     // std::printf("log status start\n");
                // }
                // else
                // {
                //     // std::printf("log status end\n");
                // }
            }
        }
    }
...
bool loadCommutationLog()
{
    ...
    // // std::printf("Commutation done " << commutation_before.count() << "seconds before .... \n");
...
bool loadZeroPoint()
{
    ...
    // {
    //     // std::printf("commutation saved time is longer then cached valid time\n");
    //     return false;
    // }

    //std::printf("ZP saved at " << std::ctime(&file_time);
    //std::printf("ZP saved at " << commutation_before.count() << "seconds before .... \n");
...
void findZeroPoint(int slv_number)
{
    ...
    if (elmofz[slv_number].findZeroSequence == FZ_CHECKHOMMINGSTATUS)
    {
        ...
        {
            //std::printf("motor " << slv_number << " init state : homming on\n");
            elmofz[slv_number].findZeroSequence = FZ_FINDHOMMINGSTART;
            elmofz[slv_number].initTime = control_time_real_;
            elmofz[slv_number].initPos = q_elmo_[slv_number];
        ...
        {
            //std::printf("motor " << slv_number << " init state : homming off\n");
