...
bool initTocabiSystem(const TocabiInitArgs &args)
{
    ...
    if (!ec_init_redundant(args.port1, args.port2))
    // if (!ec_init(ifname))
    {
...
void *ethercatThread1(void *data)
{
    ...
    printf("ELMO %d : entering initialize \n", g_init_args.ecat_device);
...
{
    int Q_START = g_init_args.ecat_slave_start_num;
    ...
    if (g_init_args.ecat_device == 1)
    atomic_store(&shm_msgs_->statusCount,cycle_count,memory_order_release);
    // TODO: 2
    if (g_init_args.ecat_device == 2)
    atomic_store(&shm_msgs_->statusCount2,cycle_count,memory_order_release);
