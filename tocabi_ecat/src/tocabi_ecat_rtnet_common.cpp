...
bool initTocabiSystem(const TocabiInitArgs &args)
{
    ...
    {
        // printf("cred << "WARNING : SLAVE NUMBER INSUFFICIENT" << creset << '\n';
        return false;
    }
...
void *ethercatThread1(void *data)
{
    ...
    while (!de_shutdown)
    {
        ...
        {
            printf("%d Lat avg : %7.3f max : %7.3f COM avg : %7.3f max : %7.3f\n", cycle_count / 2000, lat_avg / 1000.0, lmax, send_avg / 1000.0, smax);
